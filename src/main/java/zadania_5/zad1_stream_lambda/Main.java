package zadania_5.zad1_stream_lambda;
...
public class Main {
    public static void main(String[] args) {
        ...
        Company intelGda2 = new Company("Intel2", "Gdansk", 150, new ArrayList<>(Arrays.asList(
                new Purchase(productServerDellPrecision, LocalDate.of(2018, 1, 1), 50, UNIT.UNIT),
                new Purchase(productLaptopApple, LocalDate.of(2018, 1, 1), 100, UNIT.UNIT),
                new Purchase(productPhoneApple, LocalDate.of(2018, 1, 1), 5, UNIT.UNIT),
                new Purchase(productCatering, LocalDate.of(2018, 1, 1), 5, UNIT.UNIT),
        ...
        System.out.println("FIRMY Z DETROID");
        firmyDetroid(companies);

        System.out.println("FIRMY Z LONDYNU");
        firmyLodnyn(companies);


        System.out.println("FIRMY Z GDANSKA");
        firmyWarszawa(companies);

        System.out.println("FIRMA Z Kijowa z najwieksza liczba pracowników");
        employeesKijow(companies);

        System.out.println("FIRMA Z najkrotsza nazwa");
        lenghtName(companies);

        System.out.println("FIRMA nie z Kijowa, Detroin i nie z Londynu z najmniejsza iloscią zakupów");
        najmniejKupionych(companies);

        System.out.println("Dodanie pracownika");
        dodajPracownika(companies);

        System.out.println("Nazwa firmi + pracownicy");
        wysweitlenieMapy(companies);

        System.out.println("Nazwa miejscowosc + lista firm");
        wyswietlenieMapyZLIsta(companies);
    }
// Polecenie 1:
...
//        company_1_wylistuj(companies);
public static void company_1_wylistuj(List<Company> companies){
        companies.stream()
                .forEach(s->print(s));
    ...
    private static void print(Company s) {
        System.out.println(s.getName());
    }
// 2. Wylistuj wszystkie firmy które są z Detroit
    public  static void firmyDetroid (List<Company> companies){
        companies.stream()
                .filter(s->s.getCityHeadquarters().equals("Detroit"))
                .forEach(s->print(s));
    }
// 3. Wylistuj wszystkie firmy z Londynu, posortuj je po ilości pracowników (rosnąco).
    public static void firmyLodnyn (List<Company> companies){
        companies.stream()
                .filter(s->s.getCityHeadquarters().equals("London"))
                .sorted(Comparator.comparingInt(Company::getEmployees))
                //.filter(s->s.sort(s.getEmployees()))
                .forEach(s->print(s));
    }
// 4. Wylistuj wszystkie firmy z Warszawy. Posortuj je po ilości zakupów (rosnąco) i ilości pracowników (malejąco).
    public static void firmyWarszawa (List<Company> companies){

        companies.stream()
                .filter(s->s.getCityHeadquarters().equals("Gdansk"))
                .sorted(Comparator.comparingInt(s->s.getPurchaseList().size()))
                .sorted(Comparator.comparingInt(Company::getEmployees).reversed())
                .forEach(s->print(s));

    }
// 5. Zwróć firmę z największą ilością pracowników, która pochodzi z Kijowa.
    public static void employeesKijow(List<Company> companies){
        companies.stream()
                .filter(s->s.getCityHeadquarters().equals("Kijev"))
                .sorted(Comparator.comparingInt(Company::getEmployees).reversed())
                .limit(1)
                .forEach(s->print(s));
    }
// 6. Zwróć firmę z najkrótszą nazwą
    ...
    public static void lenghtName(List<Company>companies){
        companies.stream()
...
// 7. Zwróć firmę która nie pochodzi z Kijowa, Londynu i Detroit, która ma najmniej kupionych produktów.
    public static void najmniejKupionych(List<Company> companies){
        companies.stream()
                .filter(s->!s.getCityHeadquarters().equals("Kijev"))
        .filter(s->!s.getCityHeadquarters().equals("London"))
        .filter(s->!s.getCityHeadquarters().equals("Detroit"))
...
// 8. Każdej firmie dodaj po 1 pracowniku, jeśli pochodzi z Kijowa lub Detroit
    public static void dodajPracownika(List<Company>companies){

        System.out.println("Kijów");
        System.out.println("Przed daodaniem");
        companies.stream()
                .filter(s->s.getCityHeadquarters().equals("Kijev"))
        ...
        companies.stream()
                .filter(s->s.getCityHeadquarters().equals("Kijev"))
                .forEach(s-> s.setEmployees(s.getEmployees()+1));
...
public static void wysweitlenieMapy(List<Company>companies){

    Map<String,Integer>nameEmployees=companies.stream()
            .filter(s->s.getCityHeadquarters().equals("Detroit"))//dodałam poniewaz klucze musza byc unikatowe inaczej wyrzuca błąd
            .collect(Collectors.toMap(s->s.getName(),s->s.getEmployees()));
...
public static void wyswietlenieMapyZLIsta(List<Company>companies){

        Map<String ,List<Company>> miejscowoscListaFirm=companies.stream()
                                                    .collect(Collectors.groupingBy(getName-> getName.getCityHeadquarters()));

        miejscowoscListaFirm.forEach((getCityHeadquarters,getName)-> System.out.println(String.format("Miejscowosc: %s %s",getCityHeadquarters,getName)));
...
//        company_11_zakupy_najwieksze(companies);
    ...
    public static void company_11_zakupy_najwieksze(List<Company>companies){
